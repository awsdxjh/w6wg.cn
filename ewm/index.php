 ...
 */
    ...
    $text = isset($_GET['text']) ? $_GET['text'] : 'http://w6wg.cn';
	$size = isset($_GET['size']) ? intval($_GET['size']) : 800;
    if($text){
        QRcode::png($text,false,QR_ECLEVEL_L,$size,1);
    }else{
        echo '<h1>二维码API - 爱忘事的小家伙</h1>';
        echo '<hr/>';
        echo '<h4>输入参数以继续：</h4>';
        echo '<p><b>text</b>：要生成二维码的文本内容（必须）</p>';
        echo '<p><b>size</b>：生成二维码大小，单位像素</p>';
        echo '<hr/>';
        echo '<p><b>示例</b>：<a href="?text=http://w6wg.cn/ewm/&size=100">?text=http://w6wg.cn/ewm/&size=100</a></p>';
        echo '<p><b>html中调用</b>：&lt;img src="http'. (isset($_SERVER['HTTPS']) ? 's://' : '://').$_SERVER['SERVER_NAME']. $_SERVER['REQUEST_URI'].'?text=http://w6wg.cn/ewm/&size=100"&gt;</p>';
    }
